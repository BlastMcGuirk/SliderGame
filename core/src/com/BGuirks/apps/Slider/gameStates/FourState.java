...
public class FourState extends GameState {
    ...
    public FourState(GameStateManager gameStateManager) {
        super(gameStateManager);
        bg = new Background("backgrounds/FourBG.png");
    ...
    public void touchDown(int x, int y) {
        ...
        if (tm.isCompleted()){
            String best = GameStateManager.getRSM().loadDataValue("four");
            if (beats(c.getTime(), best) || best.equals("0:00")){
                GameStateManager.getRSM().saveDataValue("four", c.getTime());
