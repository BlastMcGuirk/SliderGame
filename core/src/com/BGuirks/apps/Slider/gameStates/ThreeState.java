...
public class ThreeState extends GameState {
    ...
    public ThreeState(GameStateManager gameStateManager) {
        super(gameStateManager);
        bg = new Background("backgrounds/ThreeBG.png");
    ...
    public void touchDown(int x, int y) {
        ...
        if (tm.isCompleted()){
            String best = GameStateManager.getRSM().loadDataValue("three");
            if (beats(c.getTime(), best) || best.equals("0:00")){
                GameStateManager.getRSM().saveDataValue("three", c.getTime());
